...
**IoC-контейнер** (контейнер инверсии управления) — это объект, который управляет жизненным циклом бинов, их созданием, конфигурированием и связыванием (wiring).

В Spring IoC-контейнер представлен двумя основными интерфейсами:

| Интерфейс | Роль | Ключевые возможности |
|-----------|------|----------------------|
| **`BeanFactory`** | Базовый контейнер | Ленивая инициализация, базовое управление бинами (`getBean()`, `containsBean()`) |
| **`ApplicationContext`** | Расширенный контейнер | Всё, что у `BeanFactory`, плюс: AOP, i18n, события, сканирование компонентов, работа с ресурсами |

**Основные реализации `ApplicationContext`:**
- `AnnotationConfigApplicationContext` — Java-конфигурация (`@Configuration`, `@ComponentScan`);
...
```java
// Пример программного создания контейнера
...
## Аннотации для определения бина

### Стереотипные аннотации (`@Component` и производные)

| Аннотация | Назначение | Отличия от `@Component` |
|-----------|------------|------------------------|
| **`@Component`** | Базовая стереотипная аннотация для любого Spring-бина | — |
| **`@Service`** | Сервисный слой (бизнес-логика) | Нет технических отличий, несёт только **семантическую** нагрузку |
| **`@Repository`** | DAO / доступ к данным | Добавляет **функциональное** отличие: автоматический перевод persistence-исключений в `DataAccessException` через `PersistenceExceptionTranslationPostProcessor` |
| **`@Controller`** | MVC-контроллер | Регистрируется в `HandlerMapping` как обработчик HTTP-запросов |
| **`@RestController`** | REST-контроллер | `@Controller` + `@ResponseBody` (возвращаемое значение метода сразу сериализуется в тело ответа) |
| **`@Configuration`** | Класс с фабричными методами `@Bean` | Указывает, что внутри — **программные** определения бинов, а не обычные компоненты |

**Важно:** `@Service`, `@Repository`, `@Controller` — это псевдонимы (`@AliasFor`) `@Component` с дополнительной семантикой. На этапе компонент-сканирования `ClassPathBeanDefinitionScanner` распознаёт их одинаково, но Spring-постпроцессоры могут добавлять специфическое поведение (например, для `@Repository`).

### `@Component` vs `@Bean`

| Критерий | `@Component` | `@Bean` |
|----------|-------------|---------|
| **Применяется к** | Классу | Методу |
| **Источник определения** | Компонент-сканирование (`@ComponentScan`) | Явное объявление в `@Configuration`-классе |
| **Когда использовать** | Свои классы, к которым есть доступ для аннотирования | Сторонние классы (из библиотек), которые нельзя аннотировать `@Component` |
| **Имя бина по умолчанию** | Имя класса с маленькой буквы (`UserService` → `userService`) | Имя метода |
| **Проксирование** | Обычное CGLIB-проксирование для `@Transactional` | `@Configuration` оборачивается в CGLIB-прокси, `@Bean`-методы вызываются через контекст |
| **Перегрузка** | Не применимо | Методы с одинаковым именем → один бин (переопределение невозможно) |
